function [] = Call_STK_ImageGeneration(FOV, LLA, Angles, PixelDims, FilePath)
% Jeremy Ogorzalek, 2019

% ~~ Description ~~
% Generates a 'realistic' satellite image

% ~~ Notes ~~
% Only the greater of the two half-angles defining a rectangular field of
% view will be used.

% ~~ Inputs ~~
% FOV: The maximum of the two half-angles that define the rectangular field
% of view of the satellite imager [deg]
% LLA: The lat, long, and alt of the satellite [deg, deg, m]
% Angles: Euler angles defining the orientation of the satellite [deg]
% PixelDims: The width and height resolution, in pixels, of the image to be
% generated
% FilePath: File location to save the generated image.

% ~~ Outputs ~~
% None directly. Images from STK will be saved in the file location defined
% by the 'FilePath' input
% ------------------------------------------------------------------------
...
sat.Propagator.InitialState.Representation.AssignSpherical('eCoordinateSystemFixed', Lon, Lat , Alt+6371, 90, 270, 7.72576);
...
FileName = [num2str(Lat) '_' num2str(Lon) '_' num2str(Alt) '-' num2str(Angle1) '_' num2str(Angle2) '_' num2str(Angle3)];
